...
public class AsyncSender {
	...
	@Asynchronous
	public void sendToAll(String from, String fromName, String subject, String htmlBody, String textBody, String secret,
			String groups, String services) throws Exception {

		AppParam port = this.entityManager.find(AppParam.class, AppParam.APP_MAIL_PORT);
		String mailHost = this.entityManager.find(AppParam.class, AppParam.APP_MAIL_HOST).getValue();
		String mailTrap = this.entityManager.find(AppParam.class, AppParam.APP_MAIL_TRAP).getValue();

		String mailUser = null;
		try {
			mailUser = this.entityManager.find(AppParam.class, AppParam.APP_MAIL_USER).getValue();
		} catch (Exception e1) {

		}
		String mailPass = null;
		try {
			mailPass = this.entityManager.find(AppParam.class, AppParam.APP_MAIL_PASSWORD).getValue();
		} catch (Exception e1) {
		...
		if (services != null) {
			q = this.entityManager
					.createQuery("select i from Iscritto i join i.opzioniServizi o where o.id in (:l) group by i.id")
					.setParameter("l", Arrays.asList(services.split(",")));
			rcpt.addAll(q.getResultList());
		}

		String text = from + fromName + subject + htmlBody + textBody + rcpt + groups + services;
		...
		while (this.mailMonitor.getInCorso().get(hash).peek() != null) {
			...
			try {
				...
				if (i.isTextMail() && ("" + textBody).length() > 0) {
					...
					email = new HtmlEmail();
					((HtmlEmail) email).setHtmlMsg(htmlBody.replaceAll("@@IDISCRITTO@@", i.getId()));
				}
				...
				email.send();
				this.mailMonitor.setLastLog("Invio: " + new Date() + " - " + i.getEmail());
			} catch (Exception e) {
				this.mailMonitor.setLastLog("ERRORE Invio: " + new Date() + " - " + i.getEmail());
